...
full_df = read.csv('transformed_scores.csv', header = T)
...
data1 = read.csv('imputed_data_nominal_variables(not index).csv', header = T)
...
attach(train_data)
fit <- lda(Index ~ cor11 + cor12 + cor13 + cor14
            + fir12 + fir14 + fir16 + fir18 +
            fir20 + fir22, data = train_data) # Linear discriminant function
predicted <- predict(fit, newdata = test_data)
table <- table(test_data$Index, predicted$class) # classification table
table
...
calculate_sensitivity_specificity(table)
